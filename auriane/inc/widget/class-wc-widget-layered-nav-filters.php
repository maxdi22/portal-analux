...
class JWS_Widget_Layered_Nav_Filters extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'Display a list of active product filters.', 'auriane' );
		$this->widget_id          = 'woocommerce_layered_nav_filters2';
		$this->widget_name        = __( 'Jws Active Product Filters', 'auriane' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'std'   => __( 'Active filters', 'auriane' ),
	...
	public function widget( $args, $instance ) {
		...
		if ( 0 < count( $_chosen_attributes ) || 0 < $min_price || 0 < $max_price || ! empty( $rating_filter ) ) {
			...
			if ( ! empty( $_chosen_attributes ) ) {
				foreach ( $_chosen_attributes as $taxonomy => $data ) {
					foreach ( $data['terms'] as $term_slug ) {
						...
						echo '<li class="' . esc_attr( implode( ' ', $filter_classes ) ) . '"><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'auriane' ) . '" href="' . esc_url( $link ) . '">' . esc_html( $term->name ) . '</a></li>';
					}
				}
			}
			...
			if ( $min_price ) {
				...
				echo '<li class="chosen"><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'auriane' ) . '" href="' . esc_url( $link ) . '">' . sprintf( __( 'Min %s', 'auriane' ), wc_price( $min_price ) ) . '</a></li>'; // WPCS: XSS ok.
			}

			if ( $max_price ) {
				$link = remove_query_arg( 'max_price', $base_link );
				/* translators: %s: maximum price */
				echo '<li class="chosen"><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'auriane' ) . '" href="' . esc_url( $link ) . '">' . sprintf( __( 'Max %s', 'auriane' ), wc_price( $max_price ) ) . '</a></li>'; // WPCS: XSS ok.
			}
			...
			if ( ! empty( $rating_filter ) ) {
				foreach ( $rating_filter as $rating ) {
					...
					echo '<li class="chosen"><a rel="nofollow" aria-label="' . esc_attr__( 'Remove filter', 'auriane' ) . '" href="' . esc_url( $link ) . '">' . sprintf( esc_html__( 'Rated %s out of 5', 'auriane' ), esc_html( $rating ) ) . '</a></li>';
				}
			}
        	...
        	if ( $_GET ) {
        		...
        		printf( '<li><a  href="%s" id="remove-filter-actived" class="remove-filter-actived">%s</a></li>', esc_url( $link ), esc_html__( 'Clear all' , 'auriane' ) );
        	}

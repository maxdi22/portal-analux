    ...
    if ($ranges['layout']['type'] == 'advanced') {
        ...
        foreach ($ranges as $key => $badge_settings){
            if($key !== 'layout'){
                ...
                $badge_text = (!empty($badge_settings['badge_text']) && function_exists('jws_chars_spec_html') ) ? jws_chars_spec_html($badge_settings['badge_text']) : false;
                ...
                <tr class="wdr_bulk_table_tr wdr_bulk_table_thead" style="<?php echo (!$base::$config->getConfig('table_column_header', 1) ? 'display:none' : '')?>">
                    <?php foreach ($table_sort_by_columns as $column => $order) {
                        ...
                        <th id="customize-bulk-table-title" class="wdr_bulk_table_td awdr-dragable"
                            ...
                            } ?>"><span><?php echo esc_html($tbl_title_text) ?></span>
                            </th><?php
                        ...
                        <th id="customize-bulk-table-discount" class="wdr_bulk_table_td awdr-dragable"
                            ...
                            } ?>"><span><?php echo esc_html($tbl_discount_text) ?></span>
                            </th><?php
                        } else {
                            ?>
                        <th id="customize-bulk-table-range" class="wdr_bulk_table_td awdr-dragable"
                            ...
                            }?>"><span><?php echo esc_html($tbl_range_text) ?></span></th><?php
                        }
                    }?>
                </tr>
                </thead>
                ...
                foreach ($ranges as $range) :
                    ...
                    <tr class="wdr_bulk_table_tr bulk_table_row">
                        ...
                        if (isset($range['discount_method']) && $range['discount_method'] == 'cart') {
                            $cart_discount_text = __(' (in cart)', 'auriane');
                        }
                        ...
                        if ($discount_type == "flat") {
                            ...
                            $discount_value .= __(' flat', 'auriane');
                        ...
                        if (isset($range['discount_method']) && $range['discount_method'] != 'cart') {
                            $discounted_price_for_customizer = '<span class="price_global">'.$woocommerce->formatPrice($product->get_price()).'</span>'.$woocommerce->formatPrice(isset($range['discounted_price']) ? $range['discounted_price'] : 0);
                        }else{
                        ...
                        if (isset($range['from']) && !empty($range['from']) && isset($range['to']) && !empty($range['to']) && $range['to'] == $range['from']) {
                            $discount_range = $range['to'];
                        ...
                        foreach ($table_sort_by_columns as $column => $order) {
                            ...
                            <td class="wdr_bulk_table_td wdr_bulk_title  col_index_<?php echo ''.$j;?>" data-colindex="<?php echo ''.$j;?>"
                                ...
                                <label>
                                <input type="radio" name="quantity_discount" value="<?php echo ''.$range['to'];?>"/>
                                <?php echo esc_html__('Buy ','auriane').$discount_range; ?>:</label>
                                </td><?php
                            ...
                            <td class="wdr_bulk_table_td wdr_bulk_table_discount  col_index_<?php echo ''.$j;?>" data-colindex="<?php echo ''.$j;?>"
                                ...
                                <span class="wdr_table_discounted_value" style="<?php echo ''.( !$base::$config->getConfig('table_discount_column_value', 0)) ? 'display: none' : '';?>"><?php echo esc_html__('Save ','auriane').$discount_value; ?></span>
                                ...
                                <td class="wdr_bulk_table_td wdr_bulk_range  col_index_<?php echo ''.$j;?>" data-colindex="<?php echo ''.$j;?>"
                                    style="<?php echo (!$base::$config->getConfig('table_range_column', 0) || isset($range['discount_method']) && in_array($range['discount_method'], array('product', 'cart'))) ? 'display:none':'';?>"><?php echo ''.$discounted_price_for_customizer; ?></td><?php
                            }
                            $j++;
                        }?>
                    </tr>
